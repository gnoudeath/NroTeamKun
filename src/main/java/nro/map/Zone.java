        ...
        if (mob != null) {
            ...
            if ((((Math.abs(p.x - mob.pointX) <= xSkill) && (Math.abs(p.y - mob.pointY) <= ySkill)) || (mobId == (byte) 70)) && !p.isTroi) {
                ...
                if (p.idSkillselect == 20 && p.mp >= skillPlayerUse.manaUse && !p.isdie && !mob.isDie && !p.checkPlayerBiKhongChe() && (_timeNOW - skillPlayerUse.lastTimeUseThisSkill > (long) (skillPlayerUse.coolDown))) {
                    ...
                    } else if (p.power >= 80000000000L) {
                        upSmTnDCTT = (long) (upSmTnDCTT / 15);
                    } else if (p.power >= 1800000000000L) {
                        upSmTnDCTT = (long) (upSmTnDCTT / 20);
                    } else if (p.power >= 300000000000L) {
                        upSmTnDCTT = (long) (upSmTnDCTT / 50);
                    } else if (p.power >= 500000000000L) {
                        upSmTnDCTT = (long) (upSmTnDCTT / 100);
                    } else if (p.power >= 1000000000000L) {
...
//                                itemROI.itemTemplateID = (short)itemROI.itemMapID;
                                    ...
                                    itemDrop.add(item);
                                    itemsMap.addAll(itemDrop);
                                } else if (percentDropItemGod < 40 && map.MapCold()) {
...
//                        }
                    ...
                    if (_bTarget._typeBoss == 59 && p.havePet == 0) { //boss die la ngokhong
                        p.wukhong = _bTarget.wukhong;
                        leaveDEEEEE(_bTarget.wukhong);
                        p.havePet = 1;
                        p.isNewPet = true;
                        p.wukhong.id = (-100000 - p.id);
                        p.statusPet = 0;
                        p.petfucus = 1;
                        pets.add(p.wukhong);
                        for (byte i = 0; i < players.size(); i++) {
...
//                                itemROI.itemTemplateID = (short)itemROI.itemMapID;
                        ...
                        } else if (_bTarget._typeBoss == (byte) 6) {
                            ItemMap itemM = cNewItemMap(992, p.id, _bTarget.x, _bTarget.y);   
                            if (itemM != null) {
                                addItemToMap(itemM, p.id, _bTarget.x, _bTarget.y);
                            }
                        } else if (_bTarget._typeBoss >= (byte) 31 && _bTarget._typeBoss <= (byte) 35) {
                        ...
                        } else if (_bTarget._typeBoss == (byte) 60 || _bTarget._typeBoss == (byte) 61 || _bTarget._typeBoss == (byte) 62) {//rong1sao
                            int idItemMap[] = {14, 15, 16, 17, 18, 19, 20, 21};
                            ItemMap itemM = newItemMapBuff(idItemMap[Util.nextInt(0, 10)], p.id, _bTarget.x, _bTarget.y);
                            if (itemM != null) {
                                addItemToMap(itemM, p.id, _bTarget.x, _bTarget.y);
                                addItemToMap(itemM, p.id, _bTarget.x += 15, _bTarget.y);
...
//        if(_detu.tiemNang > 0 && (_detu.hpGoc < _detu.getHpMpLimit() || _detu.mpGoc < _detu.getHpMpLimit() || _detu.damGoc < _detu.getDamLimit())) {
        ...
        if (_mob.isDie) {
            ArrayList<ItemMap> itemDrop = new ArrayList<>();
            if (_mob.template.tempId != 0) {
                int idItemNotSell[] = {17, 188, 189, 190, 18, 19, 20, 441, 442, 443, 444, 445, 446, 447,188, 189, 190, 225, 76};
                int percentDrop = Util.nextInt(0, 10);
                int percentDropItemGod = Util.nextInt(0, 5000);
                int perTA = Util.nextInt(0, 1000);
...
//                                timerSPBroly.schedule(ttSuper, 15000);
                                ...
                                timerBrolyBack.schedule(ttBrolyBack, 30000);
                            }
...
//                            leaveDEEEEE(_broly.detu);
                            ...
                            timerBrolyBack.schedule(ttBrolyBack, 60000);
                        }
                        return;
                    } else {
...
//                                    _broly.hpFull = (int)(_broly.hpFull*2);
                        ...
                        if (_charTarget != null) {
                            chat(_broly, "đố anh bắt được em");
                        }
                    }
                }
            };
...
//                        timerBOSS.cancel();
                        if (_BOSS.id > -201000) {
                            ...
                            if (_BOSS._typeBoss == 3) { //COOLER 1 DIE
                                ...
                                timerCoolerXuatHien.schedule(ttCoolerXuatHien, 15000);
                                } else if (_BOSS._typeBoss == 59) { //NgoKhong DIE
                                Timer timerNgoKhongXuatHien = new Timer();
                                TimerTask ttNgoKhongXuatHien = new TimerTask() {
                                    public void run() {
                                        int IDZONE = Util.nextInt(0, map.area.length);
                                        Boss _rNgoKhong = new Boss(48, (byte) 59, (short) 243, (short) 168);

                                        map.area[IDZONE].bossMap.add(_rNgoKhong);
                                        map.area[IDZONE].loadBossNoPet(_rNgoKhong);
                                    }
                                ;
                                };
                                timerNgoKhongXuatHien.schedule(ttNgoKhongXuatHien, 15000);
                                                ...
                                                timerBossLeave.schedule(bossLeave, 10000);
                                                    ...
                                                    for (Detu _pet : pets) {
                                                        if (Math.abs(_BOSS.x - _pet.x) < 700 && Math.abs(_BOSS.y - _pet.y) < 700) {
                                                            ...
                                                            hoiSinhDetu.schedule(hsDetu, 6000);
                                                                for (Player _pll : players) {
...
//                                }
                    ...
                    if (_charTarget != null) {
                        ...
                        } else if (_BOSS._typeBoss == (byte) 59 || _BOSS._typeBoss == (byte) 59) {
                            chat(_BOSS, "Mạnh mẽ lên, như thế này mà đòi giết ta à ??");    
                         } else if (_BOSS._typeBoss >= (byte) 60 || _BOSS._typeBoss <= (byte) 62) {
                            chat(_BOSS, "định ăn trộm ngọc rồng à ??");        
                        }
                    }
                }
            };
