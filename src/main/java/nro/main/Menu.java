            ...
            case 42: { //QUOC VUONG MO GIOI HAN SUC MANH
                if (p.map.id == 43) {
                    ...
                    if (p.menuID == 0) { //NANG GIOI HAN SUC MANH CHO SU PHU
                        if (select == 0) {
                            if (p.limitPower >= 30) {
                                doMenuArray(p, idNpc, "bố mấy ô bug game r còn đòi j", new String[]{});
                            } else {
                                if (p.power >= 18000000000L) {
                                    if (p.limitPower < 30 && p.ngoc >= 100) {
                                        p.ngoc -= 100;
                                        p.limitPower += 1;
                                        Service.gI().buyDone(p);
            ...
            case 39: {
                if (p.map.id == 5) {
                    ...
                    if (select == 0) {
                        ...
                        doMenuArray(p, idNpc, "|2|Ngươi muốn kích hoạt thành viên ư ?\b|1|Phí kích hoạt sẽ là: 20.000 KunCoin\b|1|Khi kích hoạt thành viên thành công\b|1|Ngươi sẽ nhận thêm 20 Thỏi Vàng nữa !\b|7|Số KunCoin hiện có: " + p.sotien, new String[]{"Kích Hoạt", "Từ Chối"});
                        p.menuID = select;
                        break;
                    } else if (select == 2) {
                        ...
                        doMenuArray(p, idNpc, "|2|Nhân ngày giáng sinh dragon oke 2 chúc các bạn\n|2|Có 1 ngày vui vẻ và thật nhiều hạnh phúc <3\n|7|Bên dưới là món quà dragon muốn gửi đến bạn\n|1|Chúc các bạn chơi game vui vẻ", new String[]{"Nhận Quà\nGiáng Sinh", "Từ Chối"});
                        p.menuID = select;
                        break;
                    }
                }
...
//                        }
            ...
            case 18: { //THAN MEO
               if (p.map.id == 46) {
                    if (p.menuID == -1) {
                        if (select == 0) {
                            if (Server.gI().opentranhngocnamec) {
                                int keyngoc = p.getIndexItemBagByID(457);
                                if (keyngoc != -1 && p.ItemBag[keyngoc].quantity >= 50) {
                                    p.ItemBag[keyngoc].quantity -= 50;
                                    if (p.ItemBag[keyngoc].quantity <= 0) {
                                        p.ItemBag[keyngoc] = null;
                                    }
                                    Service.gI().updateItemBag(p);
                                    GotoMap(p, 49);
                                } else {
                                    p.sendAddchatYellow("Cần 50 thỏi vàng để tiến vào đây");
                                }
                            } else {
                                p.sendAddchatYellow("chưa mở");
                            }
                        } else if (select == 1) {
                            break;
                        }
...
//                            Service.chatNPC(p, (short) 24, HelperDAO.getTopCard());
                            ...
                            if (!ServerMaintenance.isUnderMaintenance()) {
                                ...
                                p.sendAddchatYellow("Chức năng này không sử dụng được trong thời gian bảo trì");
                            }
                            break;
