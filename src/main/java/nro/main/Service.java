...
//                    m.writer().writeInt(Server.gI().menu.timeUpMagicTree(player.levelTree));
        ...
        try{
            ...
            if (item != null){
                if(item.template.id == 457) {
                    if (actionBuy == 0) {
                        if (player.vang >= Constant.MAX_MONEY) {
                            player.sendAddchatYellow("Bạn không thể bán vàng nữa.");
                            return;
                        }
                        Service.gI().SaleDone(player, typeBuy, "Bạn có muốn bán\n" + "x1 thỏi vàng với giá 500Tr vàng không?", indexBuy);
                    } else if (actionBuy == 1) {
                        player.ItemBag[(byte)indexBuy].quantity -= 1;
                        if(player.ItemBag[(byte)indexBuy].quantity == 0) {
                            player.ItemBag[(byte)indexBuy] = null;
                        }
                        Service.gI().updateItemBag(player);
                        player.vang = ((long)player.vang + Constant.GOLD_STONE) > Constant.MAX_MONEY ? Constant.MAX_MONEY : ((long)player.vang + Constant.GOLD_STONE);                       
                        Service.gI().buyDone(player);
                        player.sendAddchatYellow("Bạn nhận được 500Tr vàng");
                    } else {
    ...
    public static void createCachePart() {
        try {
            ...
            Part[] partsFromDB = ItemDAO.readPartsFromDB();
            ds.writeShort(partsFromDB.length);
            for (Part p : partsFromDB) {
                ds.writeByte(p.type);
...
//            msg.writer().writeShort(lengthHead);
            msg.writer().writeShort((short) 302); //vhalloween
...
//            msg.writer().writeShort((short)289); //v221
            ...
            //whis
            msg.writer().writeShort((short) 1161);
            msg.writer().writeShort((short) 7710);
//            msg.writer().writeShort(992);
...
public void initRong1sao() {
        int IDZONE = Util.nextInt(0, Server.gI().maps[49].area.length);
        Boss _tRong1sao = new Boss(528, (byte) 60, (short) 1335, (short) 192);

        Server.gI().maps[49].area[IDZONE].bossMap.add(_tRong1sao);
        Server.gI().maps[49].area[IDZONE].loadBossNoPet(_tRong1sao);
        sendThongBaoServer("BOSS " + _tRong1sao.name + " vừa xuất hiện tại " + Server.gI().maps[49].template.name);
        Util.log("INIT tRong1sao XONG MAP " + Server.gI().maps[49].template.name + ", " + IDZONE);
    }
public void initRong2sao() {
        int IDZONE = Util.nextInt(0, Server.gI().maps[49].area.length);
        Boss _tRong2sao = new Boss(528, (byte) 61, (short) 1335, (short) 192);

        Server.gI().maps[49].area[IDZONE].bossMap.add(_tRong2sao);
        Server.gI().maps[49].area[IDZONE].loadBossNoPet(_tRong2sao);
        sendThongBaoServer("BOSS " + _tRong2sao.name + " vừa xuất hiện tại " + Server.gI().maps[49].template.name);
        Util.log("INIT tRong2sao XONG MAP " + Server.gI().maps[49].template.name + ", " + IDZONE);
    }
public void initRongOmega() {
        int IDZONE = Util.nextInt(0, Server.gI().maps[49].area.length);
        Boss _tRongOmega = new Boss(528, (byte) 62, (short) 1335, (short) 192);

        Server.gI().maps[49].area[IDZONE].bossMap.add(_tRongOmega);
        Server.gI().maps[49].area[IDZONE].loadBossNoPet(_tRongOmega);
        sendThongBaoServer("BOSS " + _tRongOmega.name + " vừa xuất hiện tại " + Server.gI().maps[49].template.name);
        Util.log("INIT tRongOmega XONG MAP " + Server.gI().maps[49].template.name + ", " + IDZONE);
    }
